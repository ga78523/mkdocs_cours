...
!!! example "Adresse de broadcast ou de diffusion"
    L’adresse de broadcast permet d’envoyer des paquets de données dans les réseaux IP à tous les participants d’un réseau local. 
    L'adresse de diffusion se calcule par l'opération suivante : (adresse du réseau) OU logique (le complément à 1 du masque).

!!! example "Exercice 1:"
    Un ordinateur possède sur un réseau l'adresse IP suivant : 192.168.0.17. Le masque de sous-réseau est 255.255.255.0.
    
    1. Traduire l'adresse de l'ordinateur ainsi que celle du masque en binaire.
...
Les tables de routage des routeurs font très souvent apparaître deux colonnes, interface et passerelle, dont il ne faut pnfondre l'utilité :

* **interface**: Ceci représente le port de sortie local de l'appareil qui détient cette table. Dans le contexte d'une table de routage, cette colonne est souvent le nom logique de l'interface (ex. : eth0, interface1, etc.).

Toutefois, dans certaines implémentations, on renseigne parfois l'adresse IP locale de l'interface elle-même.
...
Remarque : maximum transmission unit (MTU) est la taille maximale d'un paquet pouvant être transmis en une seule fois (sans fragmentation) sur une interface sur un réseau
...
!!! example  "Exercice 5 : Protocole RIP"
...
!!! example  "Exercice 6 : Protocole OSPF"
    Soit le réseau suivant :
